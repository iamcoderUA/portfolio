...
var _adminAuth = __webpack_require__(12);
...
if (document.getElementsByClassName('welcome-auth__form').length > 0) {
	var getAdmin = (0, _adminAuth2.default)();
...
function auth() {
    ...
    var text = document.querySelector('.front-info__capt');
...
function slider() {
  ...
  var slideItems = [{ imgUrl: 'img/content/slider-1.png', slideCaption: 'Корпоративный сайт фитнес-центра', technology: 'HTML5, CSS3, БЭМ, Git', slideUrl: 'https://iamcoderua.github.io/Afrodita/' }, { imgUrl: 'img/content/slider-2.png', slideCaption: 'Лендинг ресторана бургеров', technology: 'pug, sass, JavaScript, jQuery, AJAX, Gulp4, WebPack3, Git', slideUrl: 'https://iamcoderua.github.io/burger/production/' }, { imgUrl: 'img/content/slider-3.jpg', slideCaption: 'Сайт-визитка психоаналитика, SPA (в разработке)', technology: 'pug, sass, VanillaJS, VueJS, AJAX, node.js, PHP, MongoDB, WebPack3, Git', slideUrl: '' }, { imgUrl: 'img/content/slider-4.png', slideCaption: 'Мой сайт портфолио', technology: 'pug, sass, VanillaJS, AJAX, VueJS, node.js, MongoDB, Gulp, WebPack, Git', slideUrl: 'http://iamcoder.in.ua' }];
...
function skills() {
  var mainList = document.querySelector('.skills-list');
  var mainListItem = mainList.getElementsByClassName('skills-list__item');
  var circleSec = mainList.getElementsByClassName('skills-row__circle-second');
  var listName = ['Frontend', 'Backend', 'Workflow'];
  var skills = [];
  var count = 0;
  ...
  function getSkills() {
    ...
    if (xhr.status !== 200) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        skills.push(_createElem);
      }
    }
  }

  function createCircle(arrayitem, text, circleListArray) {
    var circleClass = ['circle__first', 'circle__second'];
    var skillItem = document.createElement('li');
    var firstCircle = '<circle r="45" cx="50%" cy="50%" fill="none" class="skills-row__circle-first" />';
    var secondCircle = '<circle r="45" cx="50%" cy="50%" transform="rotate(-90 55 55)" class="skills-row__circle-second" data-name="' + text + '">';
    var svgText = '<div class="skills-row__text">' + text;
  ...
  function createElem() {
    ...
    for (var i = 0; i < mainListItem.length; i++) {
      ...
      var circleList = document.createElement('ul');
      circleList.classList.add('skills-row__list');
      circleListArray.push(circleList);

      var createheloObj = { id: index, val: mainListItem[i] };
      skillItems.push(createheloObj);
      index++;
    }
    for (var _i = 0; _i < skills.length; _i++) {
      for (var j = 0; j < skillItems.length; j++) {
        if (skills[_i].typeSkill === skillItems[j].id) {
          createCircle(skillItems[j].val, skills[_i].nameSkill, circleListArray[j]);
        }
      }
    }
  }
  ...
  return {
    skillsAnimateInit: function skillsAnimateInit() {
      getSkills();
      createElem();
...
function adminAuth() {
    ...
    function postUserInfo(form, radioLabel) {
        ...
        if (form) {
            (function () {
                ...
                var validInp = function validInp() {
                    errMes.textContent = 'Не все поля формы заполнены!';
                    errMes.classList.add('active');
                    var flag = false;
                };

                var errMes = document.querySelector('.welcome-auth__message');
                var inp = document.getElementsByTagName('input');
                var inpText = document.getElementsByClassName('welcome-auth__input');
                var user = {};
                var flag = false;;
                ...
                if (!inpText[0].value || !inpText[1].value) {
                    var _loop = function _loop(i) {
                        if (inpText[i].value === '' || inpText[i].value === ' ') {
                            var focusInp = function focusInp() {
    ...
    return {
        authInit: function authInit(form, radioClass, id) {
            var container = document.querySelector(form);
            var radioHuman = document.querySelector(radioClass);
            var radioNoHuman = document.getElementById(id);

            radioHuman.addEventListener('click', function (e) {
                radioCheckClass(radioHuman);
            });
            radioNoHuman.addEventListener('click', function (e) {
